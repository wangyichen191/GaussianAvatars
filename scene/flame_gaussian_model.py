...
class FlameGaussianModel(GaussianModel):
    def __init__(self, sh_degree : int, disable_flame_static_offset=False, not_finetune_flame_params=False, n_shape=300, n_expr=100):
        ...
        self.deformNet = Deform_Model().cuda().eval().requires_grad_(False)
        self.start_compute_offset = False

        # binding is initialized once the mesh topology is known
        if self.binding is None:
            self.binding = torch.arange(len(self.flame_model.faces)).cuda() # self.flame_model.faces (10144,3) 
    ...
    def load_meshes(self, train_meshes, test_meshes, tgt_train_meshes, tgt_test_meshes):
        if self.flame_param is None:
            ...
            num_verts = self.flame_model.v_template.shape[0] # (5143, 3)
        ...
        _, self.verts_cano_base = self.flame_model(
        ...
        self.triangles_cano = self.verts_cano_base[:, faces]
        self.face_center_cano = self.triangles_cano.mean(dim=-2)
    ...
    def update_mesh_properties(self, verts, verts_cano, flame_params):
        # verts and verts_cano (1, 5143, 3)
        faces = self.flame_model.faces # (10144, 3) 每个face由三个顶点确定
        triangles = verts[:, faces] # (1, 10144, 3, 3) 有10144个三角面片，每个三角面片由3个点组成
        # triangles_cano = self.verts_cano_base[:, faces] # (1, 10144, 3, 3) 有10144个三角面片，每个三角面片由3个点组成

        # position
        self.face_center = triangles.mean(dim=-2).squeeze(0) # （10144, 3)

        # orientation and scale
        self.face_orien_mat, self.face_scaling = compute_face_orientation(verts.squeeze(0), faces.squeeze(0), return_scale=True) # (10144, 3, 3) (10144, 1)
    ...
    def training_setup(self, training_args):
        ...
        # MLP
        self.deformNet.train().requires_grad_(True)
        param_mlp = {'params': self.deformNet.parameters(), 'lr': training_args.mlp_lr, "name": "mlp"}
        self.optimizer.add_param_group(param_mlp)
