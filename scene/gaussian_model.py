...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        self.deformNet.load_state_dict(deform_dict)
    ...
    def create_from_pcd(self, pcd : Optional[BasicPointCloud], spatial_lr_scale : float):
        ...
        if pcd == None:
            ...
            fused_point_cloud = torch.zeros((num_pts, 3)).float().cuda() # (10144, 3)
            fused_color = torch.tensor(np.random.random((num_pts, 3)) / 255.0).float().cuda() # (10144, 3)
        else:
            fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()
            fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda() # (10144, 3, 16)
        ...
        if self.binding is None:
            ...
            scales = torch.log(torch.ones((self.get_xyz.shape[0], 3), device="cuda")) # (10144, 3)
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda") # (10144, 4)
        rots[:, 0] = 1
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        if self.binding != None:
            ...
            face_scaling = self.face_scaling[self.binding[selected_pts_mask].to(torch.int64)]
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        origin = self.start_compute_offset
        ...
        self.start_compute_offset = origin
