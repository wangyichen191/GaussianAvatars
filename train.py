...
from utils.loss_utils import l1_loss, ssim, l1_regularitation
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, compute_offset_iteration):
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # when to start compute offset
        if iteration == compute_offset_iteration:
            print("Start use Offset")
            gaussians.start_compute_offset = True
        ...
        if gaussians.binding != None:
            ...
            if gaussians.start_compute_offset:
                if opt.position_offset_regularization != 0:
        ...
        with torch.no_grad():
            ...
            if (iteration in checkpoint_iterations):
                ...
                print(f"Num Points: {gaussians._xyz.shape[0]}")
                torch.save((gaussians.capture(), iteration), scene.model_path + "/chkpnt" + str(iteration) + ".pth")

            # save image
            if iteration % opt.image_save_interval == 0:
                save_image = torch.cat((gt_image.detach().unsqueeze(0), image.detach().unsqueeze(0)), dim=0)
                save_path = os.path.join(dataset.model_path, "train_image", f"{iteration}".zfill(5) + ".png")
...
def prepare_output_and_logger(args):    
    ...
    os.makedirs(os.path.join(args.model_path, "train_image"), exist_ok=True)
...
def training_report(tb_writer, iteration, losses, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
        ...
        if 'position_offset_regularization' in losses:
            tb_writer.add_scalar('train_loss_patches/position_offset_regularization', losses['position_offset_regularization'].item(), iteration)
        if 'rotation_offset_regularization' in losses:
            tb_writer.add_scalar('train_loss_patches/rotation_offset_regularization', losses['rotation_offset_regularization'].item(), iteration)
        if 'scaling_offset_regularization' in losses:
            tb_writer.add_scalar('train_loss_patches/scaling_offset_regularization', losses['scaling_offset_regularization'].item(), iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compute_offset_iteration", type=int, default=10000)
    ...
    args.save_iterations.insert(0, 10000)
    args.checkpoint_iterations.insert(0, 10000)
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.compute_offset_iteration)
